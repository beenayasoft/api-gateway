...
async def proxy_request(
    ...
    if current_user:
        ...
        library_routes = ["/api/main-oeuvre/", "/api/library/", "/api/categories/", "/api/fournitures/", "/api/ouvrages/", "/api/ingredients/"]
        if any(request.url.path.startswith(route) for route in library_routes):
            ...
            if tenant_id:
                ...
                raise HTTPException(
                    status_code=400,
                    detail="X-Tenant-ID header or tenant_id parameter is required for library endpoints"
                )
